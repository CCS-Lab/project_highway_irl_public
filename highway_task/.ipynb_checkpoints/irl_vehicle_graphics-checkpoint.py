...
class VehicleGraphics(object):
    ...
    def display(cls, vehicle: Vehicle, surface: "WorldSurface",
        ...
        roof_length, roof_width = v.LENGTH*0.3, v.WIDTH*0.55
        window_length, window_width = v.LENGTH*0.55, v.WIDTH*0.74
                
        x_adjust = -0.5 #adjust window and roof x coordinate
        ...
        rect_headlight_left = (surface.pix(tire_length+v.LENGTH-headlight_length),
                               surface.pix(length / 2 - (1.3*v.WIDTH) / 3),
        ...
        rect_headlight_right = (surface.pix(tire_length+v.LENGTH-headlight_length),
                                surface.pix(length / 2 + (1.1*v.WIDTH) / 5),
                                surface.pix(headlight_length),
                                surface.pix(headlight_width))
        color = cls.get_color(v, transparent)
        
        #headlight
        pygame.draw.rect(vehicle_surface, cls.YELLOW, rect_headlight_left, 0,border_top_right_radius=6)
        #pygame.draw.rect(vehicle_surface, cls.lighten(color), rect_headlight_left, 0,border_top_right_radius=8)
        pygame.draw.rect(vehicle_surface, cls.YELLOW, rect_headlight_right, 0,border_bottom_right_radius=6)
        
        #body filler
        pygame.draw.rect(vehicle_surface, color, rect, 0, border_top_left_radius=6,border_bottom_left_radius=6,
                 border_top_right_radius=9,border_bottom_right_radius=9)
        
        #side mirror
        pygame.draw.line(vehicle_surface, color, [4.7*v.LENGTH+x_adjust*2,5.5*v.WIDTH], [5.1*v.LENGTH+x_adjust*2,6.5*v.WIDTH], 1)   
        pygame.draw.line(vehicle_surface, color, [5.1*v.LENGTH+x_adjust*2,11*v.WIDTH], [4.7*v.LENGTH+x_adjust*2,11.5*v.WIDTH], 1)
        
        if draw_roof:
            rect_window = (surface.pix(v.LENGTH/2 - tire_length/2)+x_adjust,
                         surface.pix(1.3*v.WIDTH),
                         surface.pix(window_length),
                         surface.pix(window_width))
            pygame.draw.rect(vehicle_surface, cls.BLACK, rect_window, 0, border_radius=3)
            
            rect_roof = (surface.pix(1.2*v.LENGTH/2 - tire_length/2)+x_adjust,
                         surface.pix(1.35*v.WIDTH),
                         surface.pix(roof_length),
                         surface.pix(roof_width))
            pygame.draw.rect(vehicle_surface, cls.darken(color), rect_roof, 0)
            
            ##window section
            pygame.draw.line(vehicle_surface, color, [3.5*v.LENGTH+x_adjust*2,9.8*v.WIDTH], [5.1*v.LENGTH+x_adjust*2, 10*v.WIDTH], 1) #[2*v.LENGTH - tire_length, 2*v.WIDTH], [4*v.LENGTH - tire_length, 2*v.WIDTH], 1)
            pygame.draw.line(vehicle_surface, color, [3.5*v.LENGTH+x_adjust*2,7.4*v.WIDTH], [5.1*v.LENGTH+x_adjust*2, 7*v.WIDTH], 1)
            pygame.draw.line(vehicle_surface, color, [1.9*v.LENGTH+x_adjust*2,7*v.WIDTH], [3.5*v.LENGTH+x_adjust*2,7.4*v.WIDTH], 1)
            pygame.draw.line(vehicle_surface, color, [1.9*v.LENGTH+x_adjust*2,10*v.WIDTH], [3.5*v.LENGTH+x_adjust*2,9.8*v.WIDTH], 1)
            
        pygame.draw.rect(vehicle_surface, cls.BLACK, rect, 1, border_top_left_radius=6,border_bottom_left_radius=6,
                         border_top_right_radius=9,border_bottom_right_radius=9)
        ...
        #realistic
        if type(vehicle) in [IRLVehicle]:
            window_position = [surface.pix(tire_length), surface.pix(length / 2 - v.WIDTH / 2)]
