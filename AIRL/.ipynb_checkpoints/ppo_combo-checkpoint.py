...
class PPOTrain:
    def __init__(self, Policy, Old_Policy, gamma=0.99, lambda_1=0.97, clip_value=0.1,  lr_policy=1e-4,
                 ep_policy=1e-8, lr_value=1e-4, ep_value=2e-8, optim = 'Adam', swag = False):
        ...
        if swag:            
            self.lr_value = tf.placeholder_with_default(lr_value, shape=[], name=None)
            self.lr_policy = tf.placeholder_with_default(lr_policy, shape=[], name=None)
        else:
            self.lr_value = lr_value
            self.lr_policy = lr_policy
            print("fixed lr_value",lr_value, "lr_policy",lr_policy)
        ...
        if optim == 'Adam':
            ##Adam
            optimizer_policy = tf.train.AdamOptimizer(learning_rate=self.lr_policy, epsilon=ep_policy) #learning_rate=1e-4, epsilon=1e-5
            optimizer_value = tf.train.AdamOptimizer(learning_rate=self.lr_value, epsilon=ep_value)
            optimizer_value_v=tf.train.AdamOptimizer(learning_rate=self.lr_value, epsilon=ep_value)
        elif optim == 'SGD':
            ##SGD
            optimizer_policy = tf.train.GradientDescentOptimizer(learning_rate=self.lr_policy)
            optimizer_value = tf.train.GradientDescentOptimizer(learning_rate=self.lr_value)
            optimizer_value_v = tf.train.GradientDescentOptimizer(learning_rate=self.lr_value)

        self.train_op_policy = optimizer_policy.minimize(loss_policy, var_list=pi_trainable) #var_list: update policy; not policy_old
    ...
    def train_policy_swag(self, obs, actions, gaes,lr_policy):
        tf.get_default_session().run([self.train_op_policy], feed_dict={self.Policy.obs: obs,
                                                               self.Old_Policy.obs: obs,
                                                               self.Policy.acts: actions,
                                                               self.Old_Policy.acts: actions,
    ...
    def train_value_v_swag(self, obs, rewards, v_preds_next, lr_value):
    ...
    def train_value(self, obs, rtg):
        tf.get_default_session().run([self.train_op_value], feed_dict={self.Policy.obs: obs,
                                                                        self.Old_Policy.obs: obs,
                                                                        self.rtg_ph: rtg})
    def train_value_swag(self, obs, rtg, lr_value):
        # print("lr_value",lr_value)
        tf.get_default_session().run([self.train_op_value], feed_dict={self.Policy.obs: obs,
                                                                        self.Old_Policy.obs: obs,
                                                                        self.rtg_ph: rtg,
