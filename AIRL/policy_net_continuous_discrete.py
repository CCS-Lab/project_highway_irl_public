...
class Policy_net:
    def __init__(self, name: str, env, units_p, units_v, activation_p=tf.nn.relu,
                 activation_p_last=tf.tanh, activation_p_last_d = tf.nn.softmax, activation_v=tf.nn.relu, train_stddev=True,n_feature = 8):
        """
        :param name: string
        :param env: gym env
        """
            ...
            if discrete_env_check:
                ...
                with tf.variable_scope('policy_net'):
                    ...
                    for l_p in units_p:
                        layer_p = tf.layers.dense(inputs=layer_p, units=l_p, activation=activation_p,
                                                  kernel_initializer=initializer)
                    self.p_means = np.asscalar(env.action_space.high[0])*tf.layers.dense(inputs=layer_p, units=act_dim, activation=activation_p_last,
